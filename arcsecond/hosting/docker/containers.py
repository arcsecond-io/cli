...
def run_db_container(restart=True):
    ...
    vol = {
        str((Path(__file__).parent / "postgres").resolve()): {
...
def run_api_container(config, restart=True, do_try=True):
    ...
    env = {
        ...
        "FIELD_ENCRYPTION_KEY": config.read_key(
            "field_encryption_key", section_name=section
        ),
        # 'ARCSECOND_DATA_STORAGE': ''
    }

    vol = {"arcsecond_api_static_files": {"bind": "/home/app/static", "mode": "rw"}}

    # port = config_file_read_key('api_port', section=section)
    ports = {"8080/tcp": 8080}

    click.echo(
        f"Starting {service_name} container (be patient, this one may take a minute or two)..."
    )
    client = docker.from_env()
    client.containers.run(
        image_name,
...
def get_all_containers_status_string() -> str:
    ...
    for (
        ...
        image_status_msg = (
            "Docker image locally available."
            if image_status is True
            else "Image not yet pulled."
        )
        statuses.append(
            f'Service "{service_name}": {image_status_msg} {container_status_msg}'
        )
    return "\n".join(statuses)
